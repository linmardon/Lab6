...
function Break(pos) {
  this.posBreak = pos.plus(new Vector(-10, -10));
}
...
Level.prototype.obstacleAt = function (pos, size) {
  var xStart = Math.floor(pos.x); //rounds down somehow encompasses all of floor?
  var xEnd = Math.ceil(pos.x + size.x);
  var yStart = Math.floor(pos.y);
  var yEnd = Math.ceil(pos.y + size.y);

  //treat sides and top as walls. I want bottom as lava
  if (xStart < 0 || xEnd > this.width || yStart < 0) //last one is bottom
 return "wall";
//try returning lava for yEnd
if (yEnd > this.height)
  return 'lava';
//check each grid starting position and for possible obstacles (non nulls)
  for (var y=yStart; y<yEnd; y++)
  {
    for (var x=xStart; x<xEnd; x++)
    {
      var fieldType = this.grid[y][x];
      if (fieldType) //field type is anything that isn't null
          return fieldType;
    }
  }
}
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // find if there's obstacle like a wall in new position character moves to
  var obstacle = level.obstacleAt(newPos, this.size);
//size is width and height of character

  if (obstacle != "wall"){//check curly brackets
    this.pos = newPos;}
  
  //move only if there isn't a wall there
  
};

//lower gravity, more floaty
var gravity = 30;
var jumpSpeed = 17;
var playerYSpeed = 7;
var playerXSpeed = 10;
var resPos = new Vector(7, 2);
var breakPos = new Vector(-10, -10);

//THIS IS WHERE LAVA COMMANDS GO //
//LAVAAAAAAAA//
//AAAAAAAAA//
Player.prototype.moveY = function(step, level, keys) {
  //this is gravity stuff to accelerate falling speed
...
//floor is obstacle --- only allow jumping from floor
  if (obstacle){
...
if (obstacle === "end"){
//put stuff about winning or progressing to the next level or something. Eventually. Maybe.
 }
